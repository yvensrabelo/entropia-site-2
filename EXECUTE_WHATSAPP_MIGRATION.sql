-- ========================================
-- MIGRATION PARA WHATSAPP INTEGRATION
...
CREATE INDEX IF NOT EXISTS idx_whatsapp_messages_aluno_id ON whatsapp_messages(aluno_id);
CREATE INDEX IF NOT EXISTS idx_whatsapp_messages_status ON whatsapp_messages(status);
CREATE INDEX IF NOT EXISTS idx_whatsapp_messages_created_at ON whatsapp_messages(created_at DESC);
CREATE INDEX IF NOT EXISTS idx_whatsapp_preferences_aluno_id ON whatsapp_preferences(aluno_id);
...
DROP TRIGGER IF EXISTS update_whatsapp_preferences_updated_at ON whatsapp_preferences;
...
INSERT INTO whatsapp_templates (name, description, template, variables, type) 
SELECT * FROM (VALUES
  (
    ...
    ARRAY['nome_responsavel', 'nome_aluno', 'data']::TEXT[],
    'absence'
  ),
  (
  ...
  (
    ...
    ARRAY['nome_responsavel', 'nome_aluno', 'valor', 'vencimento']::TEXT[],
    'reminder'
  )
) AS t(name, description, template, variables, type)
WHERE NOT EXISTS (
  SELECT 1 FROM whatsapp_templates WHERE name = t.name
);
...
UNION ALL
SELECT 
  'whatsapp_automations' as tabela, 
  COUNT(*) as registros 
FROM whatsapp_automations
UNION ALL
SELECT 
  'whatsapp_preferences' as tabela, 
  COUNT(*) as registros 
